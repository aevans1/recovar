...
##### Luke's stop-gap new functions, these should be incorporated as options to existing functions here, in the future
def compute_latent_log_likelihood_no_batch_no_dets(test_pts, zs, cov_zs):
    """modified version of func compute_latent_log_likelihood, that assumes zs, cov_zs are batched beforehand, and ignores determinant normalization"""
    if zs.shape[1] != test_pts.shape[1]:
        raise ValueError(f"zs dim ({zs.shape[1]}) != test_pts dim ({test_pts.shape[1]})")
    if zs.shape[1] != cov_zs.shape[1]:
        raise ValueError(f"zs dim ({zs.shape[1]}) != cov_zs dim ({cov_zs.shape[1]})")
    ...
    log_likelihood = 0.5 * (compute_latent_quadratic_forms(test_pts.real, zs.real, cov_zs))
    return log_likelihood


def compute_latent_log_likelihood_no_dets(test_pts, zs, cov_zs, batch_size=None):
    """modified version of func compute_latent_log_likelihood, that ignores determinant normalization"""
    if zs.shape[1] != test_pts.shape[1]:
        raise ValueError(f"zs dim ({zs.shape[1]}) != test_pts dim ({test_pts.shape[1]})")
    if zs.shape[1] != cov_zs.shape[1]:
        raise ValueError(f"zs dim ({zs.shape[1]}) != cov_zs dim ({cov_zs.shape[1]})")
...
#####  END Luke's stop-gap new functions
